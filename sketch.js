...
function draw() {
  ...
  for (let i = Trails.length-1; i >= 0; i--) {
    ...
    if (p.isDead()) {
      //remove the particle
      p.ellipse.remove(); //Ellipse won't auto remove, so need to call the remove function
      Trails.splice(i, 1);
    }
  }
   ...
   */
  if(!aVector.ratio) {
 //Find unit vector first time, to calibrate the length of vector
...
Orbiter.prototype.drawOrbiter = function(){
    ...
    //Threejs only need to generate the object once, so just updating position instead of creating new object
    // ellipse(this.position.x, this.position.y, 10, 10);
    if(!this.ellipse) {

      this.ellipse = sphere(this.position.x,this.position.y,10,10)
      this.ellipse.color('gray')
    // this.ellipse = ellipse(this.position.x, this.position.y, 20, 20); //if using ellipse
    } else {
    this.ellipse.position.x = this.position.x
    this.ellipse.position.y = this.position.y
    // this.ellipse.update(); //if using ellipse
    }
